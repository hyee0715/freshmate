...
class PostControllerTest {
	...
	void create() throws Exception {
		...
		PostRes postRes = new PostRes(postId, memberId, post.getTitle(), post.getContent(), createdAt, updatedAt, recipeId, recipeWriterId, recipe.getWriter().getNickName(), recipe.getTitle(), recipe.getContent(), List.of(recipeGroceryRes1, recipeGroceryRes2), List.of(imageRes1, imageRes2));
		...
		mockMvc.perform(RestDocumentationRequestBuilders.multipart("/api/posts")
			...
			.andExpect(jsonPath("$.postCreatedAt").value(formatLocalDateTime(postRes.postCreatedAt())))
			.andExpect(jsonPath("$.postUpdatedAt").value(formatLocalDateTime(postRes.postUpdatedAt())))
			...
			.andDo(document("post/post-create",
				...
				responseFields(
					...
					fieldWithPath("postCreatedAt").type(STRING).description("게시글 생성 날짜"),
					fieldWithPath("postUpdatedAt").type(STRING).description("게시글 수정 날짜"),
	...
	void findAllByMemberId() throws Exception {
		...
		PostRes postRes1 = new PostRes(1L, memberId, post.getTitle(), post.getContent(), createdAt, updatedAt, recipeId, recipeWriterId, post.getRecipe().getWriter().getNickName(), post.getRecipe().getTitle(), post.getRecipe().getContent(), List.of(recipeGroceryRes1, recipeGroceryRes2), List.of(imageRes1, imageRes2));
		PostRes postRes2 = new PostRes(2L, memberId, post2.getTitle(), post2.getContent(), createdAt.plusMinutes(1), updatedAt.plusMinutes(1), recipeId, recipeWriterId, post2.getRecipe().getWriter().getNickName(), post2.getRecipe().getTitle(), post2.getRecipe().getContent(), List.of(recipeGroceryRes1, recipeGroceryRes2), List.of(imageRes1, imageRes2));
		...
		mockMvc.perform(RestDocumentationRequestBuilders.get("/api/posts?member-id=" + memberId)
			...
			.andExpect(jsonPath("$.posts[0].postCreatedAt").value(formatLocalDateTime(postRes1.postCreatedAt())))
			.andExpect(jsonPath("$.posts[0].postUpdatedAt").value(formatLocalDateTime(postRes1.postUpdatedAt())))
			.andExpect(jsonPath("$.posts[0].recipeId").value(postRes1.recipeId()))
			.andExpect(jsonPath("$.posts[0].recipeWriterId").value(postRes1.recipeWriterId()))
			...
			.andExpect(jsonPath("$.posts[0].recipeMaterials[0].recipeGroceryId").value(postRes1.recipeMaterials().get(0).recipeGroceryId()))
			.andExpect(jsonPath("$.posts[0].recipeMaterials[0].recipeId").value(postRes1.recipeMaterials().get(0).recipeId()))
			.andExpect(jsonPath("$.posts[0].recipeMaterials[0].recipeTitle").value(postRes1.recipeMaterials().get(0).recipeTitle()))
			.andExpect(jsonPath("$.posts[0].recipeMaterials[0].groceryId").value(postRes1.recipeMaterials().get(0).groceryId()))
			...
			.andExpect(jsonPath("$.hasNext").value(postsRes.hasNext()))
			.andDo(print())
			.andDo(document("post/post-find-all-by-member-id",
				...
				responseFields(
					...
					fieldWithPath("posts[].postCreatedAt").type(STRING).description("게시글 생성 날짜"),
					fieldWithPath("posts[].postUpdatedAt").type(STRING).description("게시글 수정 날짜"),
	...
	void update() throws Exception {
		...
		PostRes postRes = new PostRes(postId, memberId, postReq.title(), postReq.content(), createdAt, updatedAt, recipeId, recipeWriterId, recipe.getWriter().getNickName(), recipe.getTitle(), recipe.getContent(), List.of(recipeGroceryRes1, recipeGroceryRes2), List.of(imageRes1, imageRes2));
